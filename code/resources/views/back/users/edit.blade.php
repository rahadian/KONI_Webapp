      ...
      <div class="row">
        <div class="col-12">
          <div class="card mb-4">
            ...
            <form enctype="multipart/form-data" action="{{route('users.update',[$users->user_id])}}" method="POST">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select id="role" name="role" class="form-control select border rounded w-10" required>
                        @if($users->role=="admin")
                            <option value="{{ $users->role }}">Admin Pusat</option>
                        @elseif($users->role=="staff")
                            <option value="{{ $users->role }}">Admin Staff</option>
                        @else
                            <option value="{{ $users->role }}">Admin Cabor</option>
                        @endif
                        <hr>
                ...
                <div class="form-group" id="cabor-container" style="display: none;">
                    ...
                    <select id="cabor" name="cabor" class="form-control select border rounded w-10">
                        <option value="{{ $users->cabor }}">{{ $users->cabor }}</option>
                        <hr>
