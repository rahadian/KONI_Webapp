        ...
        <div class="col-12">
          <div class="card mb-4">
            ...
            @if (session('status'))
                ...
                @endif
              <h6>Data Pengurus Cabor {{ $nama_cabor }}</h6>
              @if(Auth::user()->role=="cabor")
              <a href="{{ route("pengurus_cabor.create") }}"><button type="button" class="btn btn-primary btn-fill" style="float:left;"><i class="fa fa-plus"></i> Tambah Pengurus Cabor</button></a>
              @endif
            </div>
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="align-middle text-center text-sm">
                        @if($dt->level=='KETUA')
                        <span class="badge badge-sm bg-gradient-success">{{ $dt->level }}</span>
                        @elseif($dt->level=='WAKIL KETUA')
                        <span class="badge badge-sm bg-gradient-primary">{{ $dt->level }}</span>
                        @elseif($dt->level=='SEKRETARIS')
                        <span class="badge badge-sm bg-gradient-warning">{{ $dt->level }}</span>
                        @elseif($dt->level=='BENDAHARA')
                        <span class="badge badge-sm bg-gradient-info">{{ $dt->level }}</span>
                        @else
                        <span class="badge badge-sm bg-gradient-secondary">{{ $dt->level }}</span>
                        @endif
                      </td>
                      <td>
                        <div class="d-flex px-2 py-1">
                      ...
                      <td class="align-middle text-center">
                      @if(Auth::user()->role=="cabor")
                        <a class="btn btn-fill btn-info" href="{{ route("pengurus_cabor.edit",[$dt->id_pengurus]) }}" title='View Detail'><i class="fa fa-eye"></i></a>

                        <form onsubmit="return confirm('Anda Yakin untuk Menghapus Data Ini ?')" class="d-inline" action="{{ route('pengurus_cabor.destroy',[$dt->id_pengurus]) }}" method="post">
                        @csrf
